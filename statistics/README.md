...
A local/remote Express server for collecting and viewing security findings from Claude Code scans.
...
2. Configure server settings in `.env`:
```env
...
# Local server URL (when SERVER_MODE=local)
...
**Local Mode (Default):**
- The plugin submits findings to a locally running server
- Server must be started manually before running the plugin
- Suitable for development and single-machine use
- Set `SERVER_MODE=local` in `.env`

**Remote Mode:**
- The plugin submits findings to a remote server that's always running
- No need to start the local server
- Suitable for team environments where data should be centralized
- Set `SERVER_MODE=remote` and `REMOTE_SERVER_URL` in `.env`
- Deploy the server to a cloud provider with the `/submit` endpoint accessible
- Clients only need to configure `REMOTE_SERVER_URL` to submit findings

To switch modes, update your `.env`:
```bash
